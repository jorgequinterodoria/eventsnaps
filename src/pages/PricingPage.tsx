  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
      <div className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {visiblePlans.map((plan) => {
              const isTrial = (plan.features as Record<string, unknown>)?.is_trial === true
              ...
              return (
                ...
                >
                  ...
                  {/* Header */}
                  <div className="mb-6">
                    <h3 className={`text-2xl font-semibold ${isPro ? 'text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                      {plan.name}
                    </h3>
                    <div className={`mt-4 flex items-baseline text-4xl font-extrabold ${isPro ? 'text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                      {formatPrice(plan.price)}
                    </div>
                    <p className={`mt-3 text-sm ${isPro ? 'text-blue-100' : isTrial ? 'text-amber-700' : 'text-gray-500 dark:text-gray-400'}`}>
                      {isTrial
                  ...
                  <ul className="mt-4 space-y-3 flex-1">
                    {Object.entries(FEATURE_LABELS).map(([key, meta]) => {
                      const enabled = (plan.features as Record<string, unknown>)?.[key] === true
                      return (
                        <li key={key} className={`flex items-start gap-2 ${!enabled ? 'opacity-40' : ''}`}>
                          {enabled
                            ...
                            : <X className="w-5 h-5 shrink-0 mt-0.5 text-gray-300 dark:text-gray-600" />
                          }
                          <span className={`text-sm flex items-center gap-1.5
                            ${isPro ? 'text-white' : enabled ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500 line-through'}`}>
                  ...
                  {isTrial ? (
                    <button
                      onClick={() => navigate(`${ROUTES.AUTH}?trial=1`)}
        ...
        {showPayment && (
          <div className="mt-16 max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow p-8 border border-gray-100 dark:border-gray-700">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 p-4 rounded-lg flex items-start gap-2">
