...
export default function AuthPage() {
      ...
      if (isLogin) {
        ...
        if (data?.user?.id) {
          ...
          if (trial.activated) {
            ...
            setMessage(t('auth.registerSuccessTrial'))
          } else {
            setMessage(t('auth.registerSuccessVerify'))
          }
        } else {
          setMessage(t('auth.registerSuccessVerify'))
        }
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const { error } = await insforge.auth.signInWithOAuth({
        provider: 'google',
        redirectTo: `${window.location.origin}${ROUTES.CREATE_EVENT}`
      })
      if (error) throw error
    } catch (err: unknown) {
      if (err instanceof Error) {
        ...
        setError('Error con Google Auth')
      }
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 p-8 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="email"
                  required
                  className="appearance-none block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-all dark:bg-gray-700 dark:text-gray-100"
        ...
        <div className="mt-6">
          ...
          <div className="mt-6">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
            >
